...
async function initSanity(args, context) {
  ...
  let showDefaultConfigPrompt = !defaultConfig; // Check if we have a project manifest already
  ...
  if (reconfigure) {
    ...
    const corePath = _resolveFrom.default.silent(outputPath, '@sanity/core');

    (0, _debug.default)('@sanity/core path resolved to %s', corePath || 'null');
    (0, _debug.default)('(from %s)', outputPath);
    const coreModule = (0, _dynamicRequire.default)(corePath);
  ...
  async function getOrCreateDataset(opts) {
    if (showDefaultConfigPrompt) {
      output.print('Your content will be stored in a dataset that can be public or private, depending on\nwhether you want to query your content with or without authentication.\nThe default dataset configuration has a public dataset named "production".');
    ...
    if (datasets.length === 0) {
      ...
      const name = defaultConfig ? 'production' : await (0, _promptForDatasetName.default)(prompt, {
    ...
    if (selected === 'new') {
      ...
      const newDatasetName = defaultConfig ? 'production' : await (0, _promptForDatasetName.default)(prompt, {
